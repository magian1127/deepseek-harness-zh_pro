...
| 服务监控 | 服务监控 | 关 | 双形态：左侧会话列表与设置之间保留原面板，同时在右侧边栏新增「服务监控」tab（入口在右栏引导页）；显示对话期间新启动的本地监听服务，悬停时按需解析进程，点击时在文件管理器中定位；tab 底部「基线端口」区可点击把基线端口重新纳入监控；右栏条目时间后面有三个操作按钮——排除监控（重新放入基线，无确认）、永久监控（加入设置的自定义监控项，需确认）、终止进程（普通权限尝试终止，需确认）；左栏面板不提供按钮 |
