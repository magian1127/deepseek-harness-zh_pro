...
| 服务监控 | 服务监控 | 关 | 双形态：左侧会话列表与设置之间保留原面板，同时在右侧边栏新增「服务监控」tab（入口在右栏引导页）；显示对话期间新启动的本地监听服务，悬停时按需解析进程，点击时在文件管理器中定位；tab 底部「基线端口」区可点击把基线端口重新纳入监控 |
...
- DeepSeek Harness ≥ `0.1.5-rc.1`；完整 UI 使用 `web`，Open Design stdio 使用 `open-design`，DSH 一次性任务可用 `headless`
