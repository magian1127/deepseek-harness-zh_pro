...
- 平台与环境需求（归属/定位为尽力而为）：端口扫描与探活在 Windows / macOS / Linux 都可用——Windows `netstat` 内置，macOS `netstat` 内置，Linux 需 iproute2 的 `ss`（缺失时回退 net-tools 的 `netstat`，两者都没有则面板无数据）。悬停进程归属：Windows 用系统自带 PowerShell（需 PowerShell 3+/Win8+，`Get-CimInstance`），macOS 用系统 `ps`，Linux 读 `/proc`（仅当目标进程与 DSH 运行用户一致、或对 `/proc/<pid>` 有读权限时能取到名称/路径，否则显示「未定位到监听进程」）；PID 4 内核端点的 http.sys 反查（`netsh`）为 Windows 独有。点击定位进程目录：Windows `explorer /select`、macOS Finder 均内置可用，Linux 需桌面环境的 `xdg-open`。自定义监控项若指向非回环地址，在线判定基于 TCP connect——本机存在透明代理/网络过滤组件（会对任意目标代答握手）时可能恒为在线，回环地址（127.0.0.1 / localhost）判定精确。
- 数据流：浏览器按刷新间隔轮询 `POST /dsh-zh/api/service-monitor`（请求体携带自定义监控项与当前 `intervalSec`；主机对比扫描缓存年龄——超过间隔才重扫，否则返回缓存，再逐项 TCP connect 探活自定义监控项；回环主机 + 同源信任围栏，与「删除会话」路由同一策略）；页面不可见时暂停轮询，回到前台下一轮补上。GET 同路径为调试入口（`?intervalSec=` 可选，缺省要求最新）。悬停走 `POST /dsh-zh/api/service-monitor/resolve` 按需解析并缓存归属；点击走 `POST /dsh-zh/api/service-monitor/open`，主机读取已缓存归属后执行平台 reveal 命令；路径永远来自主机进程枚举，不接受请求传入。不注册模型工具、不上传数据；自定义监控项只存浏览器 localStorage，不落主机磁盘。
