...
- 中文补全、统计样式、思考展开、默认展开行数和对话宽度只在界面语言为中文时生效。
...
| `thinkMaxLines` 默认展开行数 | 20，范围 0–200 | localStorage | 中文界面 |
...
卸载插件时，插件会恢复文本、样式、隐藏状态、对话宽度、自动展开状态以及被折叠的思考正文。
...
- 折叠只作用于思考块正文，不影响 `自动展开最新思考` 的展开行为；流式刷新时持续按上限折叠，
  避免超长思考拖慢渲染。
- `0` 表示不限制行数，此时不起作用。关闭开关、切换英文或卸载插件时还原被折叠的正文。
