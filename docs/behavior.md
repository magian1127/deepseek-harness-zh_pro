...
- 本版本（0.8.0）词典与 locale 契约按 DSH 0.1.2-alpha.1 对齐；更旧的 DSH 上中文补全部分条目会失配。
- Node.js 要求：`^22.19.0 || >=24.0.0`。
- 中文补全只在界面语言为中文时生效。统计样式、思考展开、默认展开行数、
  自动归档、会话删除按钮、服务监控在中文和英文界面都生效，只受各自开关控制，文案随界面语言切换。
  「对话宽度」功能已移除：DSH 0.1.2 起上游原生支持会话流宽度自适应与拖拽调节。
...
由「会话批量操作」开关（`batchOpsEnabled`，localStorage，默认开）控制，中文和英文界面
都生效，文案随界面语言切换。
