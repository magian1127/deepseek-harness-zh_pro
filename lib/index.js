 ...
 *    （字段 `zhPrompt`，默认 false，客户端通过 settingsScope 读写），并在
 *    `agent/pre-step` 阶段把用户文本作为一条 `user/message` 上下文消息插入
 *    （source = deepseek-harness-zh_pro，form = notice）；任何 agent preset
 *    下都生效，聊天记录出现「上下文注入 deepseek-harness-zh_pro」行；
 *    关闭或文本为空时不插入，对模型零影响。
 *
 ...
 * + agent/pre-step 双重注入：
 * 1) 把提示文本同步进 decision.assembly.sections —— 即实际发送的 system
 *    prompt。这一步发生在 SystemPrompt.assemble 完成之后，因此即使
 *    anchored-standard 的 persona 是 `complete: true`（官方组装会把其它
 *    全局 section 丢弃），第一次对话也会带上提示。
 * 2) 同时插入一条 user/message notice 上下文消息，聊天记录显示
 *    「上下文注入 deepseek-harness-zh_pro」行，展开可见注入文本。
...
function installChinesePrompt(ctx) {
  ...
  try {
    const state = { enabled: false, text: ZH_PROMPT_TEXT }
    ...
    const makePromptMessage = function () {
      if (state.enabled !== true || state.text === '') return null
    ...
    // 把提示同步进已组装的 system prompt sections。放在 persona 之前（与官方
    // -90 约定一致）；开关关闭时移除同名 section。该方法在 assemble 之后执行，
    // 因此 complete persona 无法把它丢弃。
    const syncAssemblySection = function (assembly) {
      ...
      if (state.enabled !== true || state.text === '') {
    ...
    if (systemPrompt !== null && typeof systemPrompt?.assemble === 'function') {
      ...
      try {
        ...
        warn('systemPrompt.assemble 包装失败，提示词仍会以可见上下文消息注入')
      }
    } else {
      warn('systemPrompt 服务不可用，提示词仅以可见上下文消息注入')
    }
    ...
    log(`中文优先提示已就绪（当前${state.enabled ? '开启' : '关闭'}：system prompt + 可见上下文行）`)
