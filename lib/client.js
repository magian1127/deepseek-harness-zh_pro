    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        if (typeof document !== 'undefined' && typeof MutationObserver !== 'undefined') {
          ...
          // 对话宽度：中文界面 + 大屏（≥1200px）时，按用户设定百分比
          ...
          // 仅中文界面 + thinkMaxLines>0 生效，0 表示不限制。完整原文保存在
          ...
          const thinkMaxNow = function () {
            return activeIsZh() ? settingsStore.getSnapshot().thinkMaxLines : 0
          }
          ...
          const applyThinkLinesToBody = function (body, max) {
            ...
            if (max <= 0 || current === '') {
              // 关闭/切英文/行数 0/空正文：还原完整文本，移除控件与标记。
          ...
          const runPass = function (roots) {
            ...
            const snapshot = settingsStore.getSnapshot()
            const zh = activeIsZh()
            // 思考行数折叠：关闭/切英文时先还原折叠正文，让其完整文本接受反向改写。
            if (!zh || snapshot.thinkMaxLines <= 0) restoreAllThinkLines()
            runThinkAuto()
            // 「统计全显示」是独立开关：关闭、切英文或卸载时立即清理旧样式。
            if (!zh || snapshot.statsFull !== true) undoStatsFull(document.body)
            ...
            if (zh && snapshot.thinkMaxLines > 0) applyThinkMaxLines()
            applyChatWidth()
          }
          ...
          const runObservedPass = function (roots) {
