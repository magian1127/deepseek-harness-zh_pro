/**
 * deepseek-harness-zh_pro —— 浏览器半边：DSH 中文词典修正 + 界面增强。
 * 核心是中文补全：在界面语言为「中文」时，修正 locale 词典里残留的英文
 ...
 *
 * 语言边界：
 *   - 中文补全（词典 + DOM 文本层）只在中文界面生效；用户选英文就保持英文界面。
 *   - 其余界面增强（统计全显示、自动展开思考、默认展开行数、对话宽度）与语言
 *     无关，中文/英文界面都按开关生效。
 *   - 不强制中文：用户选英文就保持英文界面，中文补全不生效。
 *   - 不做任意页面翻译：不改页面标题、不改动用户正文。
