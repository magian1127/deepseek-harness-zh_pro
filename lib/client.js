 ...
 * settingsScope 服务读写主机 settings 命名空间 `dsh-zh` 的 `zhPrompt`（开关）与
 * `zhPromptText`（注入文本）字段；主机半边包装 systemPrompt.assemble 把该文本
 * 写入实际 system prompt（首次对话即生效），并插入 user/message 上下文消息，
 * 聊天记录显示「上下文注入 deepseek-harness-zh_pro」行。
...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    const SETTINGS_DEFAULTS = { zhComplete: true, statsFull: true, chatWidthEnabled: true, chatWidth: 90 }
    const SETTINGS_ZH = {
      nav: '增强设置',
      zhComplete: '中文补全',
    ...
    // 该开关与注入文本都不走 localStorage，而是绑定官方 settingsScope 服务
    // （命名空间 `dsh-zh`，字段 `zhPrompt` 开关 / `zhPromptText` 文本），由主机
    // 半边包装 systemPrompt.assemble 写入实际 system prompt，并在 agent/pre-step
        ...
        if (typeof document !== 'undefined' && typeof MutationObserver !== 'undefined') {
          ...
          const rewrite = function (root, exact, patterns) {
            if (root.nodeType === 3) {
              ...
              if (activeIsZh()) fixStatsFull(root)
              return
            }
