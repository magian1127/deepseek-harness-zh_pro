...
const SETTINGS_ZH = {
  ...
  serviceTargetInvalid: '地址无效或非本机回环地址；支持 127.0.0.1:81 / localhost:3000 / [::1]:8080',
    zhWebSearch: '网络搜索（DuckDuckGo）',
    zhWebSearchDesc: '对话中的 web_search 工具改用免费 DuckDuckGo 后端（无需 API Key）；与智谱插件同时安装时自动优先智谱，智谱搜索失败（含敏感内容过滤）自动转 DuckDuckGo 重试；关闭后恢复 DSH 默认后端',
...
const SETTINGS_EN = {
  ...
  styleGroupDesc: 'Thinking display and stats enhancements',
  ...
  serviceTargetInvalid: 'Invalid or non-loopback address; use 127.0.0.1:81 / localhost:3000 / [::1]:8080',
    zhWebSearch: 'Web search (DuckDuckGo)',
    zhWebSearchDesc: 'Use the free DuckDuckGo backend for the web_search tool in conversations (no API key needed); when the Zhipu plugin is also installed, Zhipu is preferred and a Zhipu failure (including content filtering) automatically retries via DuckDuckGo; turn off to restore the DSH default backend',
